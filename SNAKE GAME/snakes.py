...
class Snake:
    ...
    def check_collision(self, obstacles):
        ...
        if (self.body[0].left < 0 or self.body[0].right > WIDTH or
            self.body[0].top < 0 or self.body[0].bottom > HEIGHT):
            return True

        if any(segment.colliderect(self.body[0]) for segment in self.body[1:]):
            return True

        for obstacle in obstacles:
            if self.body[0].colliderect(obstacle):
                return True
        return False
        ...
        if not game.game_over:
            if time.time() - game.last_move > game.speed:
                ...
                if game.snake.body[0].colliderect(game.food.rect):
                    ...
                    if game.food.type['effect'] == 'speed_up':
                        game.snake.speed_boost = 2
                    elif game.food.type['effect'] == 'slow_down':
                        game.snake.speed_boost = 0.5
                    game.food.generate(game.snake.body, game.obstacles)

                if game.food.check_expired():
                    game.food.generate(game.snake.body, game.obstacles)

                if game.snake.check_collision(game.obstacles):
                    game.game_over = True
                    game.save_high_score()
